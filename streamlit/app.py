...
# Compute cleaned keywords column once
all_df["clean_kws"] = build_clean_keywords(all_df)

# Build top-20 keyword list from cleaned keywords
top_kw_series = pd.Series([kw for kws in all_df["clean_kws"] for kw in kws]).value_counts()
...
all_kws_series = pd.Series([k for kws in all_df["clean_kws"] for k in kws])
